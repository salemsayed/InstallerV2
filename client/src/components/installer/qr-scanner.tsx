...
export default function QrScanner({ onScanSuccess }: QrScannerProps) {
  ...
  useEffect(() => {
    ...
    console.error = function (...args) {
      ...
      if (
        ...
        args[0].includes("No Scandit license key found")
      ) {
  ...
  const initializeScandit = async () => {
    ...
    try {
      ...
      if (typeof window === "undefined") {
        throw new Error(
          "Window object is undefined - running in non-browser environment",
        );
      }
        ...
        try {
          ...
          if (window.ScanditSDK && window.ScanditCore) {
            ...
            try {
              ...
              console.error(
                "Dynamic import of Scandit modules failed:",
                importError,
              );
          ...
          if (!window.ScanditSDK || !window.ScanditCore) {
            throw new Error(
              "Scandit modules not available after loading attempt",
            );
          }

          console.log(
            "Scandit library modules loaded and verified successfully",
          );
        } catch (moduleError) {
      ...
      try {
        console.log(
          `Requesting Scandit license key from server for user ID: ${user.id}`,
        );

        // Use the apiRequest utility which handles auth correctly
        const licenseResponse = await apiRequest(
          "GET",
          `/api/scandit-license?userId=${user.id}`,
        );
        ...
        if (!licenseData.success) {
          throw new Error(
            `Server response indicated failure: ${licenseData.message || "Unknown error"}`,
          );
        }
        ...
        console.log(
          "Using Scandit license key from API:",
          licenseKey ? "Available ✓" : "Not found ✗",
        );

        if (!licenseKey) {
          throw new Error(
            "License key was empty or null in the server response",
          );
        }
      } catch (licenseError) {
        console.error(
          "Failed to get Scandit license key from server:",
          licenseError,
        );
        throw new Error(
          `Failed to get Scandit license key: ${licenseError.message}`,
        );
      }
      ...
      try {
        ...
        settings.enableSymbology(ScanditSDK.Symbology.QR, { enabled: true });

        // Optional: enable scanning of color-inverted QR codes (white on black)
        settings
          .settingsForSymbology(ScanditSDK.Symbology.QR)
        ...
        const barcodeCapture = ScanditSDK.BarcodeCapture.forContext(
          context,
          settings,
        );
        ...
        const overlay =
          ScanditSDK.BarcodeCaptureOverlay.withBarcodeCaptureForView(
            barcodeCapture,
            view,
          );
        ...
        throw new Error(
          `Failed to create Scandit components: ${componentError.message}`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Failed to set up scan listener: ${listenerError.message}`,
        );
      }
    } catch (error) {
      console.error("Failed to initialize Scandit:", error);
      setError(
        `فشل في تهيئة الماسح الضوئي. (رمز الخطأ: SCANDIT_INIT_ERROR)\n\nتفاصيل: ${error.message}`,
      );
      return false;
    }
  };
  ...
  const startScanner = async () => {
    ...
    if (!user) {
      ...
      setError(
        "يجب تسجيل الدخول لاستخدام الماسح. (رمز الخطأ: USER_NOT_LOGGED_IN)",
      );
      setIsScanning(false);
      return;
    }
    ...
    try {
      ...
      if (!scannerContainerRef.current) {
        ...
        setError(
          "عنصر الماسح الضوئي غير موجود. (رمز الخطأ: ELEMENT_NOT_FOUND)",
        );
        setIsScanning(false);
        return;
      }
      ...
      try {
        ...
        console.log(
          `Scanner container dimensions: ${rect.width}x${rect.height}`,
        );
        ...
        await scanditRef.current.view.connectToElement(
        ...
        await scanditRef.current.camera.switchToDesiredState(
        ...
        console.error(
          "Error connecting scanner to DOM or starting camera:",
          error,
        );
        ...
        try {
          if (scanditRef.current.camera) {
            ...
            await scanditRef.current.camera.switchToDesiredState(
              (window as any).ScanditCore.FrameSourceState.Off,
            );
          }
        } catch (cleanupError) {
        ...
        setError(
          `فشل تشغيل الماسح الضوئي. (رمز الخطأ: SCANNER_CONNECT_ERROR)\n\nتفاصيل: ${error.message}`,
        );
        setIsScanning(false);
      }
    } catch (err) {
      ...
      setError(
        `فشل غير متوقع. يرجى المحاولة مرة أخرى. (رمز الخطأ: UNEXPECTED_ERROR)\n\nتفاصيل: ${err.message}`,
      );
      setIsScanning(false);
    }
  };
  ...
  const validateQrCode = async (url: string) => {
    ...
    const warrantyUrlRegex =
      /^https:\/\/warranty\.bareeq\.lighting\/p\/([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
    const shortUrlRegex =
      /^https:\/\/w\.bareeq\.lighting\/p\/([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
    ...
    if (!isValidUUIDv4(uuid)) {
      setError(
        "رمز المنتج UUID غير صالح. يرجى مسح رمز ضمان صالح. (رمز الخطأ: INVALID_UUID)\n\nالرمز المكتشف: " +
          uuid,
      );
      setIsValidating(false);
      return;
    ...
    try {
      ...
      const scanResult = await apiRequest("POST", "/api/scan-qr", {
        uuid,
        userId: user?.id,
      });
      ...
      refreshUser()
        ...
        .catch((err) =>
          console.error("Error refreshing user after scan:", err),
        );
      ...
      setError(
        `خطأ في التحقق من رمز QR. يرجى المحاولة مرة أخرى. (رمز الخطأ: VALIDATION_ERROR)\n\nتفاصيل: ${err.message || "خطأ غير معروف"}`,
      );
      setIsValidating(false);
    }
  };
  ...
  useEffect(() => {
    ...
    console.log(
      ...
      Object.keys(import.meta.env)
        .filter(
          (key) => key.startsWith("VITE_") || key === "SCANDIT_LICENSE_KEY",
        )
    ...
    if (import.meta.env.SCANDIT_LICENSE_KEY) {
      ...
      console.error(
        "Warning: No Scandit license key found in environment variables.",
      );
    }
  }, []);
          ...
          <div className="relative h-full w-full bg-black flex items-center justify-center">
            {isValidating ? (
              ...
              <>
                ...
                {/* Scanner controls */}
                <div
                  className={`absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 to-transparent ${isScanning ? "" : "bg-black/90 bottom-0 top-0 flex items-center justify-center"}`}
                >
                  {!isScanning ? (
                    <div className="flex flex-col items-center gap-6 max-w-md mx-auto p-4">
                      ...
                      <p className="text-white/70 text-center mb-6">
                        قم بمسح رمز QR الموجود على المنتج للتحقق من أصالته
                        وإضافة النقاط لحسابك
                      </p>
                      <Button
